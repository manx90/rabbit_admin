        ...
        <div className="flex-1 overflow-auto">
          <div className="relative md:p-6 p-2 space-y-6 overflow-x-hidden">
            <div className="animate-fadeIn">
              <HeaderProduct title={"Categories"} AddNew={false} />
            </div>
  ...
  return (
    <div className="flex flex-row gap-5">
      {/* <div className="flex flex-row gap-2 w-full items-center">
				...
				<label htmlFor="AllImages" className="cursor-pointer  w-full h-full">
			...
			{productInfo.imgMeasurment ? (
						...
						<label htmlFor="ImageMeasurements" className="cursor-pointer object-contain rounded-2xl w-1/3">
						<img
				...
				{productInfo.imgChart ? (
					...
					className="object-contain rounded-2xl cursor-pointer w-full h-full"
				>
						<img
      ...
      <UploadImages className="w-full h-full" />
      <UploadImages className="w-full h-full" multiple={false} />
    </div>
  );
}

function SaveCancel() {
  const { productInfo, dispatchProductInfo, mainCategoryProduct, setMessage } =
    useProduct();
  ...
  const handleSave = async () => {
    try {
      ...
      formData.append(
        "priceCover",
        productInfo.priceCover ? Number(productInfo.priceCover) : 0
      );
      formData.append("isActive", String(productInfo.isActive !== false));

      // Find category and subcategory names from IDs
      const selectedCategory = mainCategoryProduct.find(
        (cat) => cat.id === productInfo.categoryId
      );
      ...
      if (productInfo.imgCover) {
        formData.append("imgCover", productInfo.imgCover);
      }

      // Add additional images
      if (productInfo.imgs && productInfo.imgs.length > 0) {
      ...
      if (productInfo.imgMeasurment) {
        formData.append("imgMeasure", productInfo.imgMeasurment);
      }

      // Add chart image
      if (productInfo.imgChart) {
        formData.append("imgSize", productInfo.imgChart);
      }

      // Add color images with their names and sizes
      if (productInfo.ImgWithColor && productInfo.ImgWithColor.length > 0) {
        productInfo.ImgWithColor.forEach((color, colorIndex) => {
          if (color.img instanceof File) {
            formData.append("imgColor", color.img);
          }

          // Add color name
          formData.append(
            `colorsWithSizes[${colorIndex}].name`,
            color.colorName
          );

          // Add sizes for this color
          if (
            productInfo.SizeQuantityPrice &&
            productInfo.SizeQuantityPrice.length > 0
          ) {
            productInfo.SizeQuantityPrice.forEach((size, sizeIndex) => {
              // Only add size if it has a quantity for this color
              const colorQuantity = size.ColorQuantity?.find(
                (cq) => cq.colorName === color.colorName
              );
              if (colorQuantity && colorQuantity.quantity) {
                formData.append(
      ...
      if (!response.ok) {
        ...
        setMessage({
          type: "error",
          text: errorData.message || "Failed to save product",
        });
        return;
      }
    ...
    <div>
      <form>
        <div className="flex flex-col  bg-white rounded-2xl p-[12px] border">
          <div className="flex flex-row gap-5">
            <div className="flex flex-col  gap-3 w-full bg-white">
              ...
              <div className="flex flex-col gap-2">
                ...
                <select className="Inputs text-gray-500" placeholder="Name">
          ...
          <div id="size" className="flex flex-row gap-2 w-full mt-3">
            <div className="flex flex-col gap-2 w-full">
              <label htmlFor="" className="text-sm">
                Size
              </label>
  ...
  return (
    <div className="flex flex-col gap-5">
      <p className="text-[#535353] text-[18px]">
        City : <span className="text-[#0095FF]">New York</span>
      </p>
      <p className="text-[#535353] text-[18px]">
        Village : <span className="text-[#0095FF]">New York</span>
      </p>
      <p className="text-[#535353] text-[18px]">
        Address : <span className="text-[#0095FF]">New York</span>
      </p>
      <p className="text-[#535353] text-[18px]">
        Detailed Address :<span className="text-[#0095FF]">New York</span>
      </p>
    </div>
  );
}
