...
const Product = ({ categories }) => {
  ...
  const handleColorNameChange = (colorIndex, newName) => {
    setValue(`colors.${colorIndex}.name`, newName, {
      shouldDirty: true,
      shouldTouch: true,
    });

    const sizesVal = getValues("sizes") || [];
    for (let sIdx = 0; sIdx < sizesVal.length; sIdx++) {
      setValue(`sizes.${sIdx}.quantities.${colorIndex}.colorName`, newName, {
        shouldDirty: true,
      });
    }
  };
  ...
  const handleEditProduct = (item) => {
    ...
    setValue("subcategory", String(item.subCategory.id));

    // Set wordkey as array if it's a string
    const wordkeyArray = Array.isArray(item.wordkey)
      ? item.wordkey
      : item.wordkey
      ? item.wordkey.split(",").map((w) => w.trim())
      : [];
    setValue("wordkey", wordkeyArray);

    // Set colors with proper structure using replace
    if (item.colors && Array.isArray(item.colors)) {
      const colorsData = item.colors.map((color) => ({
        name: color.name || "",
        imgColor:
          typeof color.imgColor === "string" ? "" : color.imgColor || [],
      }));
      replaceColor(colorsData);
    }

    // Set sizes with proper structure using replace
    if (item.sizeDetails && Array.isArray(item.sizeDetails)) {
      const sizesData = item.sizeDetails.map((size) => ({
  ...
  useEffect(() => {
    const colorsArr = (getValues("colors") || []).map((c) => c ?? {});
    const sizesVal = getValues("sizes") || [];
    ...
    const rebuilt = sizesVal.map((size) => {
      const nextQ = colorsArr.map((c, idx) => ({
        colorName: c.name || "",
        quantity: size.quantities?.[idx]?.quantity ?? "",
      }));
      return { ...size, quantities: nextQ };
    });

    if (JSON.stringify(rebuilt) !== JSON.stringify(sizesVal)) {
  ...
  const onSubmit = async (data) => {
    ...
    if (data.date) {
      ...
      if (!isNaN(dateToSend.getTime())) {
        ...
        const dateString = dateToSend.toISOString().split("T")[0];
        formdata.append("datePublished", dateString);
      } else {
        console.warn("Invalid date provided:", data.date);
        // If date is invalid, send current date
        const now = new Date();
        formdata.append("datePublished", now.toISOString().split("T")[0]);
      }
    } else {
    ...
    formdata.append("subCategoryId", String(data.subcategory));
    ...
    data.colors.map((c, idx) => {
      formdata.append(`colors[${idx}][name]`, c.name);
    });
    data.sizes.map((s, idx) => {
      formdata.append(`sizes[${idx}][sizeName]`, s.sizeName);
    ...
    if (data.colors && Array.isArray(data.colors)) {
      data.colors.forEach((c) => {
        if (c.imgColor && c.imgColor.length > 0) {
          ...
          if (colorFile) {
            formdata.append("imgColors", colorFile);
          }
        }
      });
    }
    if (data.imgMeasure && data.imgMeasure.length > 0) {
      // Extract the actual File object from FileWithPreview wrapper
      const measureFile = data.imgMeasure[0];
      if (measureFile) {
        formdata.append("imgMeasure", measureFile);
      }
    }
    if (data.imgSizeChart && data.imgSizeChart.length > 0) {
      // Extract the actual File object from FileWithPreview wrapper
      const sizeChartFile = data.imgSizeChart[0];
      if (sizeChartFile) {
        formdata.append("imgSizeChart", sizeChartFile);
      }
    }
    ...
    try {
      ...
      if (isEditing && editingProductId) {
        await ProductAPI.update(editingProductId, formdata).then((res) => {
          console.log(res);
        });
      } else {
        ...
        {isEditing && (
          <div className="mb-4 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg">
            <h2 className="text-lg font-semibold text-blue-800 dark:text-blue-200">
              Editing Product: {watch("name") || "Loading..."}
            </h2>
            <p className="text-sm text-blue-600 dark:text-blue-300">
              Make your changes and click "Update" to save, or "Cancel Edit" to
              discard changes.
            </p>
          </div>
        )}
                      ...
                      >
                        ...
                        <SelectContent className="bg-white dark:bg-gray-700 dark:border-gray-600">
                          {categories?.data
                            ?.find(
                              (category) =>
                                String(category.id) === getValues("category"),
                            )
                    ...
                    render={({ field }) => (
                      ...
                      >
                        ...
                        <SelectContent className="bg-white dark:bg-gray-700 dark:border-gray-600">
                          ...
                          <SelectItem value="all">كل السنة</SelectItem>
                        </SelectContent>
                      </Select>
                    )}
                  />
            ...
            <Row className="w-full border-t border-gray-500 dark:border-gray-600 pt-4">
              <Column className="w-full  dark:border-gray-600 pt-4">
                ...
                <InputOne
                  ...
                  value={watch("wordkey") ? watch("wordkey").join(", ") : ""}
                />
              </Column>
              <Column className="w-full  dark:border-gray-600 pt-4">
                <Label className="text-gray-500 dark:text-gray-400">
                ...
                <DateCalender setValue={setValue} />
              </Column>
            </Row>
          </Section>
          <Section className="col-span-1 lg:max-w-[800px]">
