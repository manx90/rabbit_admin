...
export default function SizeComponent({
  ...
  const quantities = watch(`sizes.${index}.quantities`) || [];
  ...
  return (
    ...
    >
      <Row className="w-full mb-2 border-b border-gray-200 dark:border-gray-700 pb-2">
        ...
        <Column className="w-full">
          ...
          <InputOne
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                setFocus(`sizes.${index}.quantities.[0].quantity`);
                e.preventDefault();
              }
            }}
          />
      ...
      {quantities.map((color, colorIdx, arr) => (
        <Row className="w-full items-end gap-2" key={String(colorIdx)}>
          <Column className="flex-1">
            ...
            <InputOne
              type="text"
              {...register(`sizes.${index}.quantities.${colorIdx}.colorName`)}
              className="border-gray-300 bg-gray-50 dark:border-gray-600 dark:bg-gray-700"
          ...
          <Column className="flex-1">
            ...
            <InputOne
              ...
              placeholder="0"
              className="border-gray-300 bg-white dark:border-gray-600 dark:bg-gray-700"
              {...register(`sizes.${index}.quantities.${colorIdx}.quantity`)}
              defaultValue={color.quantity}
              onKeyDown={(e) => {
                if (e.key === "Enter" && arr.length - 1 > colorIdx) {
                  setFocus(
                    `sizes.${index}.quantities.${colorIdx + 1}.quantity`,
                  );
                  e.preventDefault();
                } else if (e.key === "Enter" && arr.length - 1 === colorIdx) {
                  handleAddSize();
                  e.preventDefault();
                }
              }}
